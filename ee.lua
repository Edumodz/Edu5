		...
		if tostring(string.lower(NameCallMethod)) == "kick" then
			if n == nil then
				game.StarterGui:SetCore(
					"SendNotification",
					{ Title = "KAKAZiT Hub", Text = "Kick prevented.", Duration = 2 }
				)
				print("[KAKAZiT Hub] Kick prevented.")
			else
				game.StarterGui:SetCore(
					"SendNotification",
					{ Title = "KAKAZiT Hub", Text = "Kick " .. '"' .. n .. '"' .. " prevented.", Duration = 2 }
				)
				print("[KAKAZiT Hub] Kick " .. '"' .. n .. '"' .. " prevented.")
			end
			return nil
		end
...
function MagmaUI.new()
	...
	ExitButton.MouseButton1Click:Connect(function()
		...
		wait(1)
		GUI:Destroy()
		warn('[KAKAZiT Hub] Terminating Threads (ignore errors like "attempt to call a nil value")')
		Threads:Cleanup()
		script:Destroy()
	end)
...
function MagmaUI:Notify(text: string, mode: number)
	if mode == nil or mode == 0 then
		print("[KAKAZiT Hub] " .. text)
	elseif mode == 1 then
		warn("[KAKAZiT Hub] " .. text)
	end
...
function Fly()
	...
	bv:Destroy()
	bv = nil
	LocalPlayer.Character.Humanoid.Platf
